...
class GPUPowerMonitor:
    def __init__(self, target_process=None):
        ...
        if self.target_process:
            if not self.is_process_running():
                ...
                self.list_gpu_processes()  # list running GPU processes for debugging
                sys.exit(1)
            else:
    ...
    def list_gpu_processes(self):
        """Print all GPU processes NVML sees"""
        procs = (
            pynvml.nvmlDeviceGetGraphicsRunningProcesses(self.handle)
            + pynvml.nvmlDeviceGetComputeRunningProcesses(self.handle)
        )
        print("GPU processes currently running:")
        if not procs:
            print("  None")
        for p in procs:
            try:
    ...
    def sample(self):
        ...
        for p in procs:
            ...
            # Track only target process if given
    ...
    def report(self, duration):
        total_energy_j = sum(p * self.sample_interval for _, p, _ in self.samples)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--process", type=str,
                        help="Target process name to track (substring match)")
